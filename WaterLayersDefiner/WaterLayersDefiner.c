...
int filter_water_layers(FILE* main_pdb, FILE* cleared_pdb)
{
	// Нинциализация переменных
	atom water;
	ZeroMemory(&water, sizeof(atom));

	int
		id, resSeq, substring_status, line_cursor = 0, protein_size_val = 0, line_counter = 0, minimum = -1;

	double
		x, y, z, d;

	char
		idc[6], name[4], res[6], record[7], resSeqc[4], xc[8], yc[8], zc[8], line[LINE_SIZE], ch = 0;

	// Зануляем память во сех структурах
	ZeroMemory(idc, 6); ZeroMemory(name, 4); ZeroMemory(res, 6); ZeroMemory(record, 7);
	ZeroMemory(resSeqc, 4); ZeroMemory(xc, 4); ZeroMemory(yc, 5);  ZeroMemory(zc, 6);

	while (ch = fgetc(main_pdb)) {// Начинаем переборку по всем символам файла собирая строки
		line[line_cursor] = ch;
		// При получении символа переноса строки расцениваем линию как целую строку
		if (ch == '\n') { // Начинаем обработку строки
			if (!is_protein_record(line)) { // Если строка не относится к белку, собираем все данные
				substring_status = substring(line, 7, 12, LINE_SIZE, idc, 6);
				substring_status = substring(line, 0, 6, LINE_SIZE, record, 6);
				substring_status = substring(line, 13, 17, LINE_SIZE, name, 4);
				substring_status = substring(line, 17, 21, LINE_SIZE, res, 6);
				substring_status = substring(line, 23, 27, LINE_SIZE, resSeqc, 4);
				substring_status = substring(line, 31, 38, LINE_SIZE, xc, 8);
				substring_status = substring(line, 38, 46, LINE_SIZE, yc, 8);
				substring_status = substring(line, 46, 55, LINE_SIZE, zc, 8);

				// Проверяем, что атом относится к молекулам первой линии

				water.c.x = atof(xc);
				water.c.y = atof(xc);
				water.c.z = atof(zc);

				for (int i = 0; i < protein_size_val; i++) {
					if (minimum == -1) {
						minimum = distance(&protein[i].c, &water.c);
						continue;
					}
					d = distance(&protein[i].c, &water.c);
					if( d < minimum ){
						minimum = d;
					}
				}
...
int main(int argc, char* argv[])
{
	...
	// Выполняем анализ 1, 2 и 3го водного слоя вокруг белка во всех доступных кадрах моделирования

	filter_water_layers(main_pdb_data_file, filtered_pdb_data_file);
